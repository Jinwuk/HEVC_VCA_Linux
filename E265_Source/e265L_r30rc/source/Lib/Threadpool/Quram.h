...
public:
	...
	EncGopJob(void *param, int id, pthread_mutex_t *mutex, pthread_cond_t *cond, int *bRefPicAvailable);
	//virtual ~EncGopJob() {};
	virtual ~EncGopJob();

	virtual void run(void *);
	static EncGopJob* createJob(void *param, int id, pthread_mutex_t *mutex, pthread_cond_t *cond, int *bRefPicAvailable);
