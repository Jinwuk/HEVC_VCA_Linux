...
Void TEncFrame::ETRI_CompressFrame(Int pocCurr, TComPic* pcPic, TComPicYuv* pcPicYuvRecOut, TComList<TComPic*>& rcListPic, std::list<AccessUnit>& accessUnitsInGOP, Bool bFirst, pthread_mutex_t *mutex, pthread_cond_t *cond, int *bRefPicAvailable, Bool bDefault)
#else
...
#endif
{
	...
	while(nextCUAddr<uiRealEndAddress)   	    					     		///determine slice boundaries : Multiple Slice Encoding is somewhat difficult @ 2015 5 14 by Seok
	{
		...
		em_cSliceEncoder.ETRI_compressSlice   (pcPic, ETRI_MODIFICATION_V00);	///Main Compression Function 
